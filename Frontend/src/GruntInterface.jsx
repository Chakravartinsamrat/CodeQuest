...
export default function GruntInterface({
  topic,
  level,
  npcID,
  xpgained,
  ...
  const [conversation, setConversation] = useState([
    {
      speaker: "opponent",
      text: "Challenge activated! Prepare yourself, warrior!",
    },
  ]);
  ...
  const getBackgroundForTopic = (topic) => {
    const backgrounds = {
      JavaScript:
        "bg-gradient-to-r from-yellow-900 to-yellow-600 bg-opacity-90",
  ...
  const getSceneElements = (topic) => {
    const elements = {
      JavaScript: (
        <>
          <div className="absolute top-10 left-10 text-8xl opacity-20 animate-pulse">
            {}
          </div>
          <div className="absolute bottom-20 right-10 text-9xl opacity-10 animate-bounce">
            {}
          </div>
          <div className="absolute top-1/4 right-1/4 text-7xl opacity-15 animate-spin">
            ()
          </div>
          <div className="absolute top-2/3 left-1/3 text-6xl opacity-20 animate-pulse">
            ;
          </div>
        </>
      ),
      Python: (
        <>
          <div className="absolute top-20 left-20 text-8xl opacity-20 animate-pulse">
            🐍
          </div>
          <div className="absolute bottom-30 right-20 text-7xl opacity-15 animate-bounce">
            def
          </div>
          <div className="absolute top-1/3 right-1/3 text-6xl opacity-15 animate-spin">
            :
          </div>
          <div className="absolute bottom-1/4 left-1/4 text-7xl opacity-20 animate-pulse">
            for
          </div>
        </>
      ),
  ...
  const updateXP = async () => {
    if (!userEmail || !npcID || !xpgained || xpUpdated) {
      console.error("Cannot update XP. Missing data:", {
        userEmail,
        npcID,
        xpgained,
      });
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/update-xp`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        addMessage(
          "opponent",
          `Congratulations! You've earned ${xpgained} XP from this battle!`
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isCorrect) {
        try {
          const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/update-xp`, {
      ...
      isCorrect = Array.isArray(currentQ.answer)
        ? currentQ.answer.some(
            (ans) => userInput.toLowerCase().trim() === ans.toLowerCase().trim()
          )
        ...
        if (currentQuestion < questions.length - 1) {
          ...
          setTimeout(async () => {
            ...
            if (finalScore >= questions.length / 2) {
              ...
              addMessage(
                "opponent",
                "You need to score at least 50% to earn XP. Try again!"
              );
            }
          }, 1200);
        }
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {streak >= 3 && (
                <div className="px-2 py-1 bg-orange-600 text-white rounded-full flex items-center">
                  <span className="text-yellow-300 mr-1">🔥</span> Streak:{" "}
                  {streak}
                </div>
              )}
            </div>
            ...
            {!challengeComplete ? (
              <div className="flex relative">
                ...
                <button
                  ...
                  className={`${
                    isLoading || challengeComplete
                      ? "bg-gray-600 cursor-not-allowed"
                      : "bg-indigo-700 hover:bg-indigo-600"
                  } text-white px-6 py-3 rounded-r-lg transition-colors font-bold border-2 border-indigo-700`}
                  disabled={isLoading || challengeComplete}
                >
                  SUBMIT
                </button>
                  ...
                  {challengeComplete && (
                    <div className="bg-indigo-900 border border-indigo-700 text-indigo-100 px-6 py-4 rounded-lg shadow-lg">
                      ...
                      <div className="text-lg mb-2">
                        You've conquered this challenge with a score of {score}/
                        {questions.length}!
                      </div>
